...
export default function ListingCategory() {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const offersQuery = query(listingRef, where(param.categoryName, "!=", ""), orderBy(param.categoryName), orderBy("timestamp", "desc"), limit(8));
            const offersSnap = await getDocs(offersQuery);

            const lastIndexListing = offersSnap.docs[offersSnap.docs.length - 1]
            setLastFetchedListing(lastIndexListing);

            const offersListingsData = offersSnap.docs.map(doc => ({
            id: doc.id,
            data: doc.data(),
            }));
            
            setListingsType(offersListingsData);
        } catch (error) {
    ...
    async function fetchMore() {
        try {
        const listingRef = collection(db, "listings");
        const offersQuery = query(listingRef, where(param.categoryName, "!=", ""), orderBy(param.categoryName), orderBy("timestamp", "desc"), startAfter(lastFetchedListing), limit(4));
        const offersSnap = await getDocs(offersQuery);
        ...
        setListingsType((prevState) => [...prevState, ...offersListingsData]);
        } catch (error) {
        throw error;
        } finally {
        setIsLoading(false)
  ...
  return (
    <div className="w-full bg-[#fff7d1] min-h-screen overflow-hidden pb">
      <div className="relative mt-0 w-full h-[170px] sm:h-[200px] flex justify-center">
        ...
        <div className="absolute w-[90%] md:w-[70%] h-[66px] sm:h-[85px] sm:top-[100px] top-[105px] flex align-middle text-gray-600 justify-between items-center py-5 px-5 sm:px-8 bg-gradient-to-b rounded-md from-[#f5d347] to-[#d3d3d3] z-10">
          <h4 className="text-md sm:text-xl text-center font-bold w-full p-1 border-r-2 h-full flex justify-center items-center uppercase">{param.categoryName} Listings</h4>
